...
export default function FAQSection() {
  ...
  return (
    <section id="faq" className="section-padding bg-gray-50 dark:bg-scc-dark-bg">
        ...
        <Accordion type="single" collapsible className="space-y-3 md:space-y-4">
          {faqs.map((faq, index) => (
            <div
              ...
              className={`transition-all duration-500 hover-glow ${visibleItems.includes(index) ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-8'}`}
            ...
            >
              <AccordionTrigger className="w-full text-left py-4 md:py-5 px-4 md:px-6 flex items-center min-h-[60px] hover:no-underline">
                <span className={`body-text leading-relaxed ${language === 'zh' ? 'font-chinese' : 'font-sans'}`}>
                  {language === 'zh' ? faq.questionZh : faq.question}
                </span>
              </AccordionTrigger>
              <AccordionContent className="px-4 md:px-6 pb-4 md:pb-6">
                <p className={`body-text text-gray-700 dark:text-scc-dark-text leading-relaxed ${language === 'zh' ? 'font-chinese' : 'font-sans'}`}>
