...
import { MessageSquare, Calendar, CheckCircle, CreditCard, Clock } from "lucide-react"
...
export function HowItWorks() {
  ...
  const steps = [
    {
      ...
      items: [
        ...
        language === 'zh' ? '24小时内收到回复' : 'Receive response within 24 hours',
    ...
    },
    {
      ...
      items: [
        ...
        language === 'zh' ? '安全支付 (Stripe/PayPal)' : 'Secure payment (Stripe/PayPal)',
  ...
  return (
    <section ref={sectionRef} id="how-it-works" className="py-20 lg:py-32 bg-white dark:bg-scc-dark-bg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className={`text-3xl sm:text-4xl md:text-5xl text-gray-900 dark:text-scc-dark-text mb-4 animate-fade-in-up ${language === 'zh' ? 'font-chinese' : 'font-sans'}`} style={{ fontWeight: 700 }}>
          ...
          {steps.map((step, index) => (
            ...
            >
              <div className="flex flex-col items-center text-center p-4 bg-white dark:bg-scc-dark-card rounded-xl shadow-lg hover:shadow-2xl transition-all duration-300 group h-full min-h-[400px]">
                ...
                {/* Content */}
                <h3 className={`text-lg md:text-xl font-bold mb-3 ${language === 'zh' ? 'font-chinese' : 'font-sans'}`}>{step.title}</h3>
                <div className={`text-gray-600 dark:text-scc-dark-text-secondary leading-relaxed text-sm md:text-base space-y-1 flex-1 flex flex-col justify-center ${language === 'zh' ? 'font-chinese' : 'font-sans'}`}>
                  {step.items.map((item, idx) => (
                    <p key={idx} className="hover:text-scc-primary transition-colors duration-200">{item}</p>
                  ))}
                </div>
              </div>
              ...
              {index < steps.length - 1 && (
                <div className={`hidden lg:block absolute top-8 -right-4 text-scc-primary/30 animate-fade-in-right ${
                  visibleElements.has(index) ? 'animate-visible' : ''
